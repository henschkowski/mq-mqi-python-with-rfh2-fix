...
class RFH2(MQOpts):
    ...
    initial_opts = [
        ["_StrucId", CMQC.MQRFH_STRUC_ID, "4s"],
    ...
    ]  # type: List[List[Union[str, int, bytes]]]

    big_endian_encodings = [
        CMQC.MQENC_INTEGER_NORMAL,
        CMQC.MQENC_DECIMAL_NORMAL,
        CMQC.MQENC_FLOAT_IEEE_NORMAL,
        CMQC.MQENC_FLOAT_S390,
        # 17
        CMQC.MQENC_INTEGER_NORMAL + CMQC.MQENC_DECIMAL_NORMAL,
        # 257
        CMQC.MQENC_INTEGER_NORMAL + CMQC.MQENC_FLOAT_IEEE_NORMAL,
        # 272
        CMQC.MQENC_DECIMAL_NORMAL + CMQC.MQENC_FLOAT_IEEE_NORMAL,
        # 273
        CMQC.MQENC_INTEGER_NORMAL
        + CMQC.MQENC_DECIMAL_NORMAL
        + CMQC.MQENC_FLOAT_IEEE_NORMAL,
    ...
    def add_folder(self, folder_data):
        ...
        try:
            ...
            raise PYIFError(
                "RFH2 - XML Folder not well formed. Exception: %s" % str(e)
            ) from e
    ...
    def unpack(self, buff, encoding=None):
        ...
        if len(buff) < 36:
            raise PYIFError(
                "RFH2 - Buffer too short. Should be 36+ bytes instead of %s" % len(buff)
            )
        ...
        if len(buff) > 36:
            if self["StrucLength"] > len(buff):
                raise PYIFError(
                    "RFH2 - Buffer too short. Expected: %s Buffer Length: %s"
        ...
        while s:
            ...
            # Check that the folder is valid xml and get the root tag name.
            try:
                g = re.match(b"^<([a-zA-Z0-9:_][a-zA-Z0-9:_\\.-]*)", folder_data)
                if g:
                    folder_name = str(g[1])
